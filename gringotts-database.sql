...
SELECT
	(CASE
		...
		WHEN wd.age BETWEEN 11 AND 10 THEN '[11-20]'
		WHEN wd.age BETWEEN 21 AND 10 THEN '[21-30]'
		WHEN wd.age BETWEEN 31 AND 10 THEN '[31-40]'
		WHEN wd.age BETWEEN 41 AND 10 THEN '[41-50]'
		WHEN wd.age BETWEEN 51 AND 10 THEN '[51-60]'
        ELSE '[61+]'
    END) AS `age_group`,
...
SELECT 
	...
	FROM wizzard_deposits w2
    WHERE w2.id = w1.id + 1) AS guest_wizzard_deposit,
    (SELECT w1.deposit_amount - 
		(SELECT w2.deposit_amount
        FROM wizzard_deposits w2
        WHERE w2.id = w1.id + 1)) AS difference
