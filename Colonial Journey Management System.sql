...
VALUES (
			(SELECT concat(year(birth_date),
						IF (birth_date > '1980-01-01',
							concat(day(birth_date), left(ucn, 4)),
                            concat(month(birth_date), right(ucn, 4))))
            FROM colonists WHERE id = 96),
			(CASE
				WHEN 96 % 2 = 0 THEN 'Pilot'
				WHEN 96 % 3 = 0 THEN 'Cook'
				ELSE 'Engineer'
			END
			), 96, (SELECT left(ucn, 1) FROM colonists WHERE id = 96)
		),
        (
			(SELECT concat(year(birth_date),
					IF (birth_date > '1980-01-01',
							concat(day(birth_date), left(ucn, 4)),
                            concat(month(birth_date), right(ucn, 4))))
            FROM colonists WHERE id = 97),
			(CASE
				WHEN 97 % 2 = 0 THEN 'Pilot'
				WHEN 97 % 3 = 0 THEN 'Cook'
				ELSE 'Engineer'
			END
			), 97, (SELECT left(ucn, 1) FROM colonists WHERE id = 97)
        ),
        (
			(SELECT concat(year(birth_date),
					IF (birth_date > '1980-01-01',
							concat(day(birth_date), left(ucn, 4)),
                            concat(month(birth_date), right(ucn, 4))))
            FROM colonists WHERE id = 98),
			(CASE
				WHEN 98 % 2 = 0 THEN 'Pilot'
				WHEN 98 % 3 = 0 THEN 'Cook'
				ELSE 'Engineer'
			END
			), 98, (SELECT left(ucn, 1) FROM colonists WHERE id = 98)
        ),
        (
			(SELECT concat(year(birth_date),
					IF (birth_date > '1980-01-01',
							concat(day(birth_date), left(ucn, 4)),
                            concat(month(birth_date), right(ucn, 4))))
            FROM colonists WHERE id = 99),
			(CASE
				WHEN 99 % 2 = 0 THEN 'Pilot'
				WHEN 99 % 3 = 0 THEN 'Cook'
				ELSE 'Engineer'
			END
			), 99, (SELECT left(ucn, 1) FROM colonists WHERE id = 99)
        ),
        (
			(SELECT concat(year(birth_date),
					IF (birth_date > '1980-01-01',
							concat(day(birth_date), left(ucn, 4)),
                            concat(month(birth_date), right(ucn, 4))))
            FROM colonists WHERE id = 100),
			(CASE
				WHEN 100 % 2 = 0 THEN 'Pilot'
				WHEN 100 % 3 = 0 THEN 'Cook'
				ELSE 'Engineer'
			END
			), 100, (SELECT left(ucn, 1) FROM colonists WHERE id = 100)
        );
...
WHERE '2019-01-01' - c.birth_date < 30
GROUP BY ss.name
ORDER BY ss.name;
...
BEGIN
	IF ((SELECT id FROM spaceships WHERE name = spaceship_name) IS NULL) THEN
