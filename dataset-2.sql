...
CREATE TABLE files (
    ...
    CONSTRAINT fk_files_parent_file
...
VALUES (concat('Critical Problem With ', (SELECT name FROM files WHERE id = 46),'!'),
		'open',
        ceil((46 * 2) / 3),
        (SELECT c.contributor_id FROM files f JOIN commits c ON f.commit_id = c.id WHERE f.id = 46)),
        (concat('Critical Problem With ', (SELECT name FROM files WHERE id = 47),'!'),
		'open',
        ceil((47 * 2) / 3),
        (SELECT c.contributor_id FROM files f JOIN commits c ON f.commit_id = c.id WHERE f.id = 47)),
        (concat('Critical Problem With ', (SELECT name FROM files WHERE id = 48),'!'),
		'open',
        ceil((48 * 2) / 3),
        (SELECT c.contributor_id FROM files f JOIN commits c ON f.commit_id = c.id WHERE f.id = 48)),
        (concat('Critical Problem With ', (SELECT name FROM files WHERE id = 49),'!'),
		'open',
        ceil((49 * 2) / 3),
        (SELECT c.contributor_id FROM files f JOIN commits c ON f.commit_id = c.id WHERE f.id = 50)),
        (concat('Critical Problem With ', (SELECT name FROM files WHERE id = 50),'!'),
		'open',
        ceil((50 * 2) / 3),
        (SELECT c.contributor_id FROM files f JOIN commits c ON f.commit_id = c.id WHERE f.id = 50));

UPDATE repositories_contributors
SET repository_id  = (SELECT repository_id
